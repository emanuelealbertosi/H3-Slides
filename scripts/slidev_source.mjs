...
try{
  ...
  await page.setContent('<!doctype html><meta charset="utf-8"><style>'+slideCSS+'body{margin:0}</style>'+
    project.slides.map((s,i)=>slideHTML(project,s,i,visualFor(project,s.content,s).image?placeholder:'')).join(''));
  ...
  rendered=rendered.map((r,i)=>r.html.replace(placeholder,'./assets/'+visualFor(project,project.slides[i].content,project.slides[i]).image));
...
project.slides.forEach((s,i)=>{
  ...
  const image=visualFor(project,s.content,s).image;

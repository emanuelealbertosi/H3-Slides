...
def _route_between(start, end, boxes):
...
def route_connection(source, target, elements):
    clearance = .16
    anchors = lambda element: [
        (element.x-element.width/2-clearance, element.y),
        (element.x+element.width/2+clearance, element.y),
        (element.x, element.y-element.height/2-clearance),
        (element.x, element.y+element.height/2+clearance),
    ]
    ...
    preferred = (
        ((source.x+sign*(source.width/2+clearance), source.y),
         (target.x-sign*(target.width/2+clearance), target.y))
        if horizontal else
        ((source.x, source.y+sign*(source.height/2+clearance)),
         (target.x, target.y-sign*(target.height/2+clearance)))
    )
    pairs = [preferred]
    alternatives = sorted(
        ((start, end) for start in anchors(source) for end in anchors(target)
         if (start, end) != preferred),
        key=lambda pair: math.dist(*pair),
    )
    pairs.extend(alternatives)
    ...
    for start, end in pairs:
        route = _route_between(start, end, boxes)
        if route:
            return route

...
test('V2 nested layout, all paragraphs, literal code and fluid height',async()=>{
  const browser=await chromium.launch({headless:true});try{
    ...
    assert.equal(report.overflow,false);assert(report.height>720);
...
test('V2 PDF and editable native PowerPoint export',async()=>{
  ...
  const pdf=await buildExports(project,dir,dir,'pdf');
  assert((await fs.stat(pdf)).size>1000);
  const pptx=await buildExports(project,dir,dir,'pptx');
